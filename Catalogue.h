...
//----------------------------------------------------- Méthodes publiques
	...
	void Rechercher(const char* a, const char* b); 
	//recherche simple: a = départ; b =arrivée
	void Rechercher2(const char* a, const char* b);  
	// recherche avancée: a = départ; b =arrivée
...
protected:
//----------------------------------------------------- Méthodes protégées
	void AddTrajet(Trajet* A); 
	// ajouter un trajet au catalogue
	void ReadLine(string thisString, string token []);
    void cherche(int i,const char* a, const char* b);
    bool Deja_ajoute(Trajet ** tableau, int taille, Trajet * traj); 
    // verifie si on a deja ajoute un dit trajet a un tableau de trajet dont on connait la taille
    void AjoutTrajSimp(string token[]);
    // ajouter un trajet simple - automatique
	...
	void AjoutTrajComp(string token[],ifstream &fic); 
	// ajouter un trajet composé - automatique
	void AjoutTrajComp(Trajet** Tab,int pos,string token[],ifstream &fic); 
	// ajouter un trajet composé qui est contenu dans un trajet composé - automatique
	void Sauteligne(int& skip,ifstream &fic); 
	// sauter les nombre de lignes spécifié par skip
	bool checkVille(string token [], int SelecVille, string villes []); 
	// verifier que le trajet satisfait les critères de séléction de villes
	bool checkIntervalle(string token [], int intervalle []); 
	//verifier que le trajet satisfait les critères d'intervalle
//----------------------------------------------------- Attributs protégés

	Trajet** Tab_trajet; 
	// un tableu qui contient des pointeurs sur tous les trajets contenus dans ce catalogue
	int count; 
	// le nombre de trajets contenus dans ce catalogue
	int sizeTab;  
	// la taille maximale du tableau Tab_trajet
	int num_results; 
	// le nombre de resultats trouvé par recherche 2
};
